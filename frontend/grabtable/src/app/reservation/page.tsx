    ...
    (async () => {
      ...
      try { 
        ...
        if(response.ok){
            ...
            const response2 = await fetch(`http://localhost:8000/v1/stores/${data.storeId}/menus`, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: "Bearer " + session.formData['access_token'],
                },
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-between p-8">
        <Suspense fallback={<div>Loading...</div>}>
          <ReactQueryProvider>
            {hasReservation ? <MyReservation menus={menus} storeID={storeID} isHost={isHost} /> : <NoReservation />}
        </ReactQueryProvider>
        </Suspense>
      <Toaster />
    </div>
